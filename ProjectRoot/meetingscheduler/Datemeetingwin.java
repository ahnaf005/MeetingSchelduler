...
public class Datemeetingwin {

    Label meetings[], persons[], times[];
    Button details[], cancels[];
    Stage stage;
    String date;

    Datemeetingwin(String date) {
        this.date = date;
    }
...
//        gridpane.add(datestring, 2, 0);
        ...
        timestring.setMinSize(200, 50);
        timestring.getStyleClass().add("label2");
        gridpane.add(timestring, 2, 0);
        ...
        if (size > 0) {
            //local = MeetingScheduler.receive.ob.get(idx).meetinglist;
        ...
        for (int i = 0; i < size; i++) {
            if (local.get(i).ob.date.equals(date)) {
...
//            gridpane.add(dates[i], 2, i + 1);
                ...
                times[i].setMinSize(200, 50);
                times[i].getStyleClass().add("label2");
                gridpane.add(times[i], 2, i + 1);
        ...
        userinfo.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                MeetingScheduler.datemeeting.close();
                ...
                delay.setOnFinished(event2 -> {
                    MeetingScheduler.datemeeting = new Datemeetingwin(date);
            ...
            if (local.get(i).ob.date.equals(date)) {
                ...
                details[i].setOnAction(new EventHandler<ActionEvent>() {
                    ...
                    public void handle(ActionEvent event) {
                        Finalmeeting tmp;
                        tmp = local2.get(idx2);
                        MeetingScheduler.val=2;
                        MeetingScheduler.event = new EventRemind(tmp,idx2,date);
                        try {
...
//                            for(int j=0;j<MeetingScheduler.receive.ob.size();j++)
//                            {
//                                if(MeetingScheduler.receive.ob.get(j).username.equals(tmp.ob.meetername))
//                                {
//                                    for(int k=0;k<MeetingScheduler.receive.ob.get(j).meetinglist.size();k++)
//                                    {
//                                        if(tmp.equals(MeetingScheduler.receive.ob.get(j).meetinglist.get(k)))
//                                        {
//                                            MeetingScheduler.receive.ob.get(j).meetinglist.remove(k);
//                                        }
//                                    }
//                                }
//                            }
